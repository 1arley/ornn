...
description: Performs exhaustive product-driven research across frontend UI libraries before implementing or redesigning interfaces, mapping real product flows to concrete components, interactions, motion patterns and accessibility requirements.
license: MIT
metadata:
...
Transformar redesign, UI, UX, animação e frontend em pesquisa orientada pelo produto:
...
O `research-router` encaminha tarefas de composição frontend para esta skill. Depois
da implementação, execute ou considere, registrando N/A com justificativa:
`ux-review`, `interaction-design`, `animation-review`, `visual-quality-review`,
`accessibility-review` e, em React, `react-doctor-audit`.
...
Para Mesa Redonda, o mapa só está completo após cobrir Home/Feed, Story, Profile,
Notifications, Navigation, Search, Filters, Comments, Replies, Bookmarks, Reactions,
Dialogs, loading/empty/error/success e mobile navigation. Avalie explicitamente:
`feed → story`, `story → back/tab/reaction/bookmark/share/comment/author profile`,
`comment → reply`, `profile → story`, `notifications → source`, `notification → read`,
`search → results`, `filters → results`, `mobile navigation → route` e
...
Somente implemente após os entregáveis de pesquisa estarem completos. Preserve a
arquitetura, tokens e stack do produto; adicione dependências apenas quando o ganho
justificar custo, licença e performance. Para React, valide versão, rendering model,
SSR/client boundaries e import path atual. Construa primeiro primitivas e tokens
compartilhados, depois fluxos e telas; forneça fallbacks responsivos, touch e reduced
motion. Adapte sempre a identidade.

## Phase 10 — Verification

Verifique os fluxos do Product Interaction Map e cada estado inventariado, incluindo
repetição, reversão, interrupção, loading, falha e viewport móvel. Depois aplique:

```text
design-library-research → implementation
  → ux-review
  → interaction-design
  → animation-review
  → visual-quality-review
  → accessibility-review
  → react-doctor-audit (React)
```
...
Use os templates em `templates/`. Para Mesa Redonda, o Product Interaction Map e a
Candidate Matrix devem cobrir explicitamente as telas e fluxos da Phase 1; um
placeholder vazio não satisfaz o critério.
