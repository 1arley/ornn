...
export function resolveItem(root, identifier) {
  const normalized = identifier.replace(/^(skills?|patterns?|recipes?|collections?|commands?):/, "");
  const items = loadLibrary(root);
...
export function resolveSkillSelection(root, names = []) {
  ...
  const add = (name) => {
    ...
    if (item.type === "skill") skills.add(item.id);
      ...
      for (const child of [...item.skills, ...item.recipes, ...item.collections]) add(child);
